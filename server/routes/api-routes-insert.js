...
module.exports = function (app) {
    ...
    app.post("/api/add/course", requireAuth, (req, res, next) => {
        const reqData = req.body
    ...
    app.post("/api/add/instructor", requireAuth, (req, res, next) => {
        const reqData = req.body
    ...
    app.post("/api/add/student", requireAuth, (req, res, next) => {
        const reqData = req.body
        db.person.findOrCreate({
            where: reqData
        }).then(data => res.status(200).json(data))
    ...
    app.post("/api/add/course-instructor", requireAuth, (req, res, next) => { // Checked!
        const reqData = req.body
    ...
    app.post("/api/add/course-student", requireAuth, (req, res, next) => {
        const reqData = req.body
    ...
    app.post("/api/set/course/online", requireAuth, (req, res, next) => {
    ...
    app.post("/api/set/course/onsite", requireAuth, (req, res, next) => {
        db.course.findOne({ where: { id: req.body.courseId } })
            .then(data => {
                db.onlineCourse.findOrCreate({
                    where: {
